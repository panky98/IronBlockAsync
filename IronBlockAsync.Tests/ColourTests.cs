...
namespace IronBlock.Tests
{
  ...
  {
    [TestMethod]
    public async Task Test_Colour_Picker()
    {
      const string xml = @"
<xml>
...
";
    ...
    [TestMethod]
    public async Task Test_Colour_Random()
    {
      const string xml = @"
<xml>
...
";
      ...
      var output = await program.Evaluate() as string;

      Assert.AreEqual(7, output.Length);
      Assert.AreEqual('#', output[0]);

      // assure it's random by asking for another one
      var output2 = await program.Evaluate() as string;
      Assert.AreNotEqual(output, output2);
    }


    [TestMethod]
    public async Task Test_Colour_Rgb()
    {
      const string xml = @"
<xml>
...
";
    ...
    [TestMethod]
    public async Task Test_Colour_Blend()
    {
      const string xml = @"
<xml>
