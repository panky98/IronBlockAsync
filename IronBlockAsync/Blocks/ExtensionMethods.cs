...
namespace IronBlock.Blocks
{
  ...
  {
    public static async Task<object> Evaluate(this IEnumerable<Value> values, string name, Context context)
    {
      var value = values.FirstOrDefault(x => x.Name == name);
      if (null == value) throw new ArgumentException($"value {name} not found");

      return await value.Evaluate(context);
    }
    ...
    public static Task<object> Evaluate(this Workspace workspace, IDictionary<string, object> arguments = null)
    {
      var ctx = new Context();
      if (null != arguments)
      {
