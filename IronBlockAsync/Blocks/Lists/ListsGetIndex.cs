...
namespace IronBlock.Blocks.Lists
{
  public class ListsGetIndex : IBlock
  {
    ...
    {
      var values = (await this.Values.Evaluate("VALUE", context)) as List<object>;
      ...
      {
        case "FROM_START":
          index = Convert.ToInt32(await this.Values.Evaluate("AT", context)) - 1;
          break;

        case "FROM_END":
          index = values.Count - Convert.ToInt32(await this.Values.Evaluate("AT", context));
          break;
