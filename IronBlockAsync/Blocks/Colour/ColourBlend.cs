...
namespace IronBlock.Blocks.Colour
{
  public class ColourBlend : IBlock
  {
    ...
    {
      var colour1 = ((await this.Values.Evaluate("COLOUR1", context)) ?? "").ToString();
      var colour2 = ((await this.Values.Evaluate("COLOUR2", context)) ?? "").ToString();
      var ratio = System.Math.Min(System.Math.Max((double)(await this.Values.Evaluate("RATIO", context)), 0), 1);
